...
export const PROJECTS = [
  ...
  },
  {
    ...
    tech: ["React", "TypeScript", "Tailwind CSS", "C#", "ASP.NET Core", "Resend", "Docker"],
    categories: ["fullstack", "frontend", "ui"],
    ...
    details: {
      PT: {
        overview: "Este portfólio pessoal — o site que você está visitando — foi desenvolvido com foco em identidade visual coerente, performance e estética minimalista suíça. Além do design, o projeto evoluiu para uma arquitetura fullstack real: frontend React na Vercel, formulário de contato via Serverless Function com Resend e uma API .NET dedicada que serve estatísticas ao vivo do GitHub.",
        problem: "Portfólios de desenvolvedores tendem a ser genéricos ou excessivamente técnicos, sem personalidade visual, e raramente comprovam na prática as tecnologias que dizem dominar. O desafio foi criar algo que comunicasse habilidade técnica, cuidado estético e demonstrasse — no próprio site — a stack de backend, frontend e integração com serviços externos.",
        solution: "Design system próprio construído sobre Tailwind CSS v4 com tokens customizados, animações via IntersectionObserver e roteamento com React Router v7. No backend, uma API em ASP.NET Core faz proxy da API pública do GitHub com cache in-memory, e uma Vercel Edge Function isolada cuida do envio de e-mail com honeypot e rate limit, mantendo chaves fora do bundle do cliente.",
        architecture: "Frontend em React 18 + TypeScript + Vite, hospedado na Vercel. Contato via Vercel Edge Function chamando a Resend API. API .NET 10 em contêiner Docker hospedada no Render, expondo /api/github/stats com IHttpClientFactory, IMemoryCache (10 min), Response Caching HTTP e rate limit por IP. Falha graciosa: se a API .NET dorme (free tier), a seção do GitHub simplesmente não é renderizada e o site segue funcional.",
        learnings: ["Arquitetura híbrida: frontend estático + serverless function + API .NET dedicada em ambientes distintos", "Cache server-side com IMemoryCache e Response Caching para proteger cotas de APIs externas", "Consumo de APIs externas com IHttpClientFactory e desserialização com System.Text.Json", "Segurança em formulários: chaves apenas no ambiente do servidor, honeypot, rate limit por IP, sanitização de input", "Design system escalável com Tailwind CSS v4 e tokens @theme customizados", "React Router v7 com Outlet context para estado compartilhado entre rotas"],
      },
      EN: {
        overview: "This personal portfolio — the site you are visiting — was built with a focus on visual coherence, performance, and Swiss minimalist aesthetics. Beyond the design, it evolved into a real fullstack architecture: React frontend on Vercel, contact form powered by a Serverless Function using Resend, and a dedicated .NET API serving live GitHub statistics.",
        problem: "Developer portfolios tend to be generic or overly technical, lacking visual personality, and rarely prove in practice the technologies they claim to master. The challenge was to communicate technical skill, aesthetic care, and demonstrate — on the site itself — the frontend, backend, and third-party integration stack.",
        solution: "A custom design system built on Tailwind CSS v4 with custom tokens, IntersectionObserver-based animations and React Router v7 routing. On the backend, an ASP.NET Core API proxies the public GitHub API with in-memory caching, and an isolated Vercel Edge Function handles email sending with honeypot and rate limiting, keeping secrets out of the client bundle.",
        architecture: "Frontend in React 18 + TypeScript + Vite, hosted on Vercel. Contact via a Vercel Edge Function calling the Resend API. .NET 10 API in a Docker container hosted on Render, exposing /api/github/stats with IHttpClientFactory, IMemoryCache (10 min), HTTP Response Caching, and per-IP rate limiting. Graceful degradation: if the .NET API is asleep on the free tier, the GitHub section simply does not render and the site remains fully functional.",
        learnings: ["Hybrid architecture: static frontend + serverless function + dedicated .NET API across separate environments", "Server-side caching with IMemoryCache and Response Caching to protect external API quotas", "Consuming external APIs with IHttpClientFactory and System.Text.Json deserialization", "Form security: secrets kept only on the server side, honeypot, per-IP rate limiting, input sanitization", "Scalable design system with Tailwind CSS v4 and custom @theme tokens", "React Router v7 with Outlet context for shared cross-route state"],
      },
    },
  },
